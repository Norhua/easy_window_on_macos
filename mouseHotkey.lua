...
local function normalizeMods(mods)
  ...
  for _, m in ipairs(MOD_ORDER) do
    if seen[m] then table.insert(normalized, m) end
  end
...
local function normalizeButton(button)
  ...
  button = button:lower()
  assert(BUTTON_EVENT_MAP[button], 'unsupported button: ' .. button .. ' (supported: left, right, middle)')
  return button
end
...
local function rebuildTap()
  ...
  if bindingCount() == 0 then return end

  MouseHotkey._tap = hs.eventtap.new(collectEventTypes(), function(event)
...
function M.hotkeyBindWithMouseDown(mods, button, fn)
  ...
  assert(MouseHotkey._bindings[bindingKey] == nil, 'mouse hotkey already exists for: ' .. bindingKey)
...
--- 向后兼容的便捷接口
function M.hotkeyBindWithLeftMouseDown(mods, fn) return M.hotkeyBindWithMouseDown(mods, 'left', fn) end
...
function M.getButtons()
  ...
  for buttonName, pressed in pairs(_pressedButtons) do
    if pressed then btns[buttonName] = true end
  end
  return btns
end
